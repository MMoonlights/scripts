...
local Window = Rayfield:CreateWindow({
    Name = "LSRS Hub",
    LoadingTitle = "LSRS Hub",
    LoadingSubtitle = "by LSRS",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = nil,
        FileName = "LSRS Hub"
...
local AuraSection = MainTab:CreateSection("Killaura")

local KillauraToggle = AuraSection:CreateToggle({
    Name = "Enabled",
    CurrentValue = false,
    Flag = "KillauraToggle",
...
local DistanceSlider = AuraSection:CreateSlider({
    Name = "Distance",
    Range = {0, 50},
    Increment = 1,
    Suffix = "studs",
...
local TeleportToggle = AuraSection:CreateToggle({
    Name = "Teleport",
    CurrentValue = false,
    Flag = "TeleportToggle",
    Callback = function(Value)
...
local TargetSection = MainTab:CreateSection("Target")

local MobDropdown = TargetSection:CreateDropdown({
    Name = "Mob To Target",
    Options = ValidMobs,
    CurrentOption = "Any",
    Flag = "MobDropdown",
...
local PrefixDropdown = TargetSection:CreateDropdown({
    Name = "Target Prefix",
...
local MinLevelBox = TargetSection:CreateInput({
    Name = "Minimum Level",
    PlaceholderText = "1",
...
local TargetLevelToggle = TargetSection:CreateToggle({
    Name = "Only Target Our Level",
    CurrentValue = false,
    Flag = "TargetLevelToggle",
    Callback = function(Value)
...
local RebirthToggle = RebirthSection:CreateToggle({
    Name = "Auto Rebirth",
    CurrentValue = false,
    Flag = "RebirthToggle",
    Callback = function(Value)
...
local AutoEquipToggle = FarmSection:CreateToggle({
...
local UIKeybind = UISection:CreateKeybind({
    Name = "Toggle UI",
...
local PopupsToggle = UISection:CreateToggle({
...
if Mobs then
    Mobs.ChildAdded:Connect(function(v)
        ...
        if not table.find(ValidMobs, Name) then
            table.insert(ValidMobs, Name)
            Rayfield:UpdateDropdownOptions("MobDropdown", ValidMobs)
        end
        
        if Prefix and not table.find(ValidPrefixes, Prefix) then
            table.insert(ValidPrefixes, Prefix)
            Rayfield:UpdateDropdownOptions("PrefixDropdown", ValidPrefixes)
        end
    end)
end
